...
**Governed Clarification (PATCH_PI_RT_CALCULUS_062):**
RT means **Real Truth**. RT is the classification label applied to an admissibly closed triad at the resolved scope of that closure. The displayed root expression is the canonical **RT_core** whole-expression binding and is therefore a foundational RT at that scope. The token RT does not perform closure; closure licenses classification as RT.
...
Within the affect/effect domain, $A|E$ remains the primitive distinction condition and must not be substituted into $RT_{core}$. The formal projection operator $\Pi_D$ maps $A|E$ into the representational distinction schema $D(*|*)$ by the law $D(*|*) := \Pi_D(A|E)$. $\Pi_D$ is a projection operator, not a generative primitive; projection preserves the typed projection route into the representational distinction schema without exhausting the primitive affect/effect condition. Whether a declared representable distinction is semantically preserved remains a separate D-semantics obligation. Accordingly, $D(*|*)$ is non-primitive and remains domain-local to the affect/effect projection layer; the projection layer is RT-governed without itself being primitive.
...
Here RT is not treated as a pre-given target. The admissibility of $x$ and the constitution of RT are mutually co-produced through residue-conditioned closure traceable to the Core Expression. The recursive engine is then expressed as $RT_n \to D(RT_n \mid continuation)$. Trace Inheritance is campaign-supported under coherent nesting controls, Continuation Admissibility is campaign-supported under typed continuation controls, and Base Case Constructive Trace is campaign-supported under base-field trace controls; all three remain `PASSED_PENDING_RIGOR_ENDORSEMENT` pending independent rigor endorsement.
...
$$ <a>_b $$
Here $D(a)$ is retained only as a context-suppressed abstraction whose omitted conditioning must be recoverable whenever it participates in governed RT reasoning. The meta-relation family therefore defaults to conditioned operands, so expressions of the form $<a>_b \langle * \rangle_x <c>_d$ are treated as the primitive compositional layer, while arithmetic juxtaposition remains a lawful projection and not the primitive ontology. Conditioning composition is campaign-supported when typed operands, directional order, conditioning lineage, admissibility context, and non-collapse remain recoverable; order-erasing or context-collapsing composition remains inadmissible. Under those same controls, asymmetry and typed residue are campaign-supported as stable candidate invariants, not as unqualified universal invariants. Conditioned equivalence is likewise supported only as non-collapsing equivalence, not as process identity. A non-arithmetic admissibility measure is campaign-supported only as a conditioning-lineage proxy for admissibility filtering outside arithmetic projection, not as a universal metric or replacement for every arithmetic projection. Conditioning propagation is campaign-supported only as lineage-preserving relational update over typed conditioned distinctions, not as physical motion, causal spacetime propagation, or a universal propagation law.

**Semantic Process Indexing Note (RT_INDUCTION_PACKET_PROCESS_SEMANTIC_INDEX_001):**
The current induction packet treats semantic indices as governed identifiers for resolved RT organizations and their orientation-field / deviation-signature projections. In this textbook, that reading is provisional and non-quantitative: a semantic index is not a numeric value, coordinate, or measurement, and it does not create authority, resolve ambiguity, or replace any live RT surface. The assignment procedure remains open and is tracked separately in Appendix F and the lexicon gap queue.
...
2.  **Relational Tension:** It maintains the non-collapse relation with Effect ($E$) via the relation $A <\neq> E$. Under PATCH_PI_RT_CALCULUS_063, when new canonical material renders this relation in ratio form under a declared reference basis, the governed abbreviation is **AER** (Affect-Effect Ratio). **RT** remains reserved for Real Truth.
...
**Provisional Projection Route (APPENDIX_F_DEVIATION_TO_GEOMETRY_RECOVERY_REDUCTION_001):**
The Appendix F deviation-to-geometry obligation is reduced to an explicit projection route:
$$ Dev_{RT} \xrightarrow{\Pi_{dist}[R,B,O,A_{adm}]} Dist_{like} \xrightarrow{Org_a} OrgDist \xrightarrow{\Pi_{geo}} geometry_{app} $$
Here $Dev_{RT}$ is a declared RT deviation class, $\Pi_{dist}$ is a reference-boundary-orientation-admissibility-relative projection into a distance-like representation, $OrgDist$ is an organized distance-like projection preserving trace metadata, and $\Pi_{geo}$ is the existing topology-to-geometry legality gate. This route does not make distance primitive, does not supply units or scalar metric structure by default, and does not treat $geometry_{app}$ as physical space or an ontological floor. If reference, boundary, orientation, admissibility, distinction, or closure trace metadata is missing, the route fails rather than producing lawful geometry.
...
- $Organism <\neq>_{bio} Subsystem$
- $Observer <\neq>_{obs} Observed$
- $A <\neq> E$ (Affect-Effect pre-distinction relational tension; when represented in new canonical material as a reference-relative ratio form, use `AER`, not `RT` or `RTen`)
...
**Deviation-to-Geometry Recovery Link (APPENDIX_F_DEVIATION_TO_GEOMETRY_RECOVERY_REDUCTION_001):**
The governed route into this legality transform may begin from a declared deviation class only through the provisional chain $Dev_{RT} \to_{\Pi_{dist}} Dist_{like} \to_{Org_a} OrgDist \to_{\Pi_{geo}} geometry_{app}$. The final step remains subject to the existing $\Pi_{\text{geo}}$ trace requirements: distinction class, admissibility status, orientation class, and closure class must remain recoverable. This is a projection route, not a proof of geometry recovery.
...
- **RT Calculus Foundation (PATCH_PI_RT_CALCULUS_001):** [ **C1_DEFINED_PROVISIONAL** ] RT_core is the unique primitive calculus of the framework. In governed usage, every primitive is RT-governed and traces to RT_core, but not every RT-governed expression is primitive; derived continuations, projections, operational regimes, and other governed RT surfaces remain RT without being primitive. Linear and nonlinear operational behavior are derived from RT_core rather than treated as independent primitives or axiomatic floors, and any local linear/nonlinear language remains regime-level shorthand downstream of RT_core. [Source: PATCH_PI_RT_CALCULUS_001].
...
- **Affect ($A$):** [ **C1_DEFINED_PROVISIONAL_MAINTAINED** ] The necessity condition for non-null distinction organization. It establishes the relational requirement for distinction before selection, maintaining a non-collapse relational tension with Effect ($E$). Under PATCH_PI_RT_CALCULUS_063, any new canonical ratio-form rendering of that tension must use `AER` (Affect-Effect Ratio) under a declared reference basis rather than `RT` or `RTen`. [Source: MPF_ADV_HARDENING_SERIES_003_AFFECT_EFFECT_BACKFILL; PATCH_PI_RT_CALCULUS_063].
...
- **Directed Relational Distinction ($D$):** [ **C1_DEFINED_PROVISIONAL** ] The notation and typed projection route are synchronized. Evaluation domain/codomain is discharged, context thresholds carry bounded support, typed projection preservation is discharged within bounded human-reviewed type-level scope, and P127/P128 provide bounded human-approved support for representable distinction and non-collapse. Universal preservation and universal threshold semantics remain outside scope under `DEBT_D_SEMANTICS_PROOF_001`.
- **Local Admissibility Floor ($\epsilon$):** [ **C1_DEFINED** ]
...
- **Symmetry-Condition Relation and Dominant-Domain Projection:** [ **C1_DEFINED_PROVISIONAL** ] `RT_SYMMETRY_CONDITION_RELATION_CORRECTION_001` corrects the relation signature to `| : BoundedSymmetry × UnboundedSymmetry -> SymmetryCondition`, with `<S>|>S< = S`; `(*|*)` is the role-sensitive notation for `S` as distinction-permitting. AX-R01 and AX-R02 are provisionally formalized; AX-R03 is operator-local `INAPPLICABLE`, AX-R04 permits only canonical self-reference and governed alias normalization, AX-R05 self-composition is `INAPPLICABLE`, AX-R06 records operator-local `RESOLVED_NOT_CLOSED`, AX-R07 provisionally formalizes validation-only malformed-construction rejection, AX-R08 is `PARTIALLY_RESOLVED_BLOCKED`, AX-R09 is `RESOLVED_INAPPLICABLE`, AX-R10 is `RESOLVED_INAPPLICABLE`, and AX-R11 formalizes the ordered domain `BoundedSymmetry × UnboundedSymmetry` with executable totality blocked. Whole-RT identity/composition/associativity/commutativity and SymmetryCondition-to-operand projection remain separate open obligations; canonical projection selection, executable semantics, AX-R12, and `[Asym]` domain compatibility remain unestablished. Superseded constructor/output-type entries are retained as aliases in `registry/lexicon_gap_queue.json`.
- **Bounded Symmetry:** [ **C1_DEFINED_PROVISIONAL** ] `RT_BOUNDED_SYMMETRY_DEFINITION_001` defines `<S>` relative to `(D,A_D,B_D,∂B_D)`, with an explicit effective bound and typed boundary interface. Boundedness is not identified with finiteness, closure, isolation, or physical ontology. Bound taxonomy, boundary calculus, constraint effectiveness, dynamic and recursive boundedness, relation to `>S<`, and executable semantics remain open; the core term is inducted provisionally in `registry/lexicon_gap_queue.json`.
- **Unbounded Symmetry:** [ **C1_DEFINED_PROVISIONAL** ] `RT_UNBOUNDED_SYMMETRY_DEFINITION_001` defines `>S<` as an internal condition with no intrinsic size or substance and non-exhaustible distinction capacity under declared local accounts. It is not numerical, spatial, material, undefined, or unconstrained infinity. Distinction-capacity semantics, local-account quantification, non-exhaustibility tests, no-size/no-substance typing, bounded-unbounded relations, and executable semantics remain open; core terms are inducted in `registry/lexicon_gap_queue.json`.
...
- **OQ_002 (Bell Curve Derivability):** [ **SCOPED_DISPOSITION_STATISTICAL_PROJECTION_REQUIRED** ] Bell-curve emergence is not formally derived from $\text{RT}_{\text{core}}$ alone. A normal-like projection requires a declared $\iff_s$ projection map $F_s$, observation window or ensemble, normalization policy, aggregation rule, and convergence or symmetry assumptions. Skewed, heavy-tailed, multimodal, and non-convergent outputs remain admissible alternatives until ruled out by declared conditions. Evidence ceiling remains `C1_DEFINED_PROVISIONAL`; theorem promotion is not authorized.
- **OQ_003 (Skew Conditions):** [ **SCOPED_DISPOSITION_ASYMMETRY_RATIO_CLASSIFIER_PENDING_FORMAL_F_s** ] Skew is governed by declared $\iff_s$ projection of directional imbalance, not by $\text{RT}_{\text{core}}$ alone. Under declared orientation/sign metadata, $\Omega_a = 1$ marks balance, $\Omega_a > 1$ marks exclusion-dominant imbalance, and $\Omega_a < 1$ marks addition-dominant imbalance. The signed $F_s$ projection, observation-axis convention, mixed-regime aggregation, and positive/negative skew fixtures remain open. Evidence ceiling remains `C1_DEFINED_PROVISIONAL`; no universal distribution-shape claim is authorized.
- **OQ_004 (Biconditional Ladder Resolution):** [ **SCOPED_DISPOSITION_RECURSIVE_SCALE_REPRESENTATION** ] The biconditional ladder is a provisional bounded representation of nested RT expressions mediated by deviation and residue-governed continuation under declared admissibility and trace conditions. PRIN_007 constrains family membership and primary roles mutually but does not establish a universal recursive-scale law. Evidence ceiling remains `C1_DEFINED_PROVISIONAL`; theorem promotion is not authorized.
- **Deviation-to-Geometry Recovery:** [ **SCOPED_DISPOSITION_PROJECTION_ROUTE_ONLY** ] The staged route $Dev_{RT} \xrightarrow{\Pi_{dist}[R,B,O,A_{adm}]} Dist_{like} \xrightarrow{Org_a} OrgDist \xrightarrow{\Pi_{geo}} geometry_{app}$ is admissible as a metadata-preserving bounded projection protocol when reference, boundary, orientation, admissibility, distinction, and closure-trace metadata are present. Missing metadata causes route failure. This does not prove geometry recovery, physical geometry, or metric universality; evidence ceiling remains `C1_DEFINED_PROVISIONAL`.
...
- **L116 Mechanized Syntax Closure:** [ **CONSTRUCTIVELY_DISCHARGED** ] Lean 4 proof pilot (`proofs/lean/MpfClosurePilot.lean`) constructs concrete `State`, `Residue`, `ContextBase`, `Operator`, `Relation`, and `Projection` base structures, eliminating base-type opacity. `Term` is a closed 6-constructor inductive syntax tree and `theorem L116_syntax_closure` compiles and verifies by `rfl` without `sorry`. Constructor classification lemmas (`L116_state_class` .. `L116_projection_class`) are explicitly categorized as regression guards / non-claims. [Source: `proofs/lean/MpfClosurePilot.lean`].
- **L117 Mechanized Failure Predicate & Valuation:** [ **CONSTRUCTIVELY_DISCHARGED_SUBSTANTIVE** ] Lean 4 proof pilot (`proofs/lean/MpfClosurePilot.lean`) defines `Context` (with `adm_floor`, `residue_active`, `is_valid`, `orientation`) and `Value` (with `zero_state` and `valid_distinction`). `Fails (t : Term) (c : Context) : Prop` is defined independently and resolved for invalid context, zero floor, frame misalignment, and inactive residue. `theorem L117_boundary_condition` compiles without `sorry` as a substantive theorem. [Source: `proofs/lean/MpfClosurePilot.lean`].
- **Option B Orientation Space & Frame Alignment:** [ **CONSTRUCTIVELY_DISCHARGED_SUBSTANTIVE** ] Lean 4 proof pilot (`proofs/lean/MpfClosurePilot.lean`) formalizes `DiscreteOrientation` ($N \ge 3$ relational crossings per T001) and `Orientation` frame alignment. Proves `orientation_failure_boundary` and `step_orientation_alignment_preservation` without `sorry`. [Source: `proofs/lean/MpfClosurePilot.lean`].
- **Option A Process Step Deduction & Invariant Conservation:** [ **CONSTRUCTIVELY_DISCHARGED_SUBSTANTIVE** ] Lean 4 proof pilot (`proofs/lean/MpfClosurePilot.lean`) constructs process transition relation `inductive Step : Term → Context → Term → Context → Prop`. Proves three mechanized invariant theorems without `sorry`: `step_admissibility_preservation`, `step_residue_accumulation`, and `step_valuation_soundness`. [Source: `proofs/lean/MpfClosurePilot.lean`].
- **P110 Projection Signature Preservation:** [ **VERIFIED_SUBSTANTIVE** ] `theorem P110_projection_signature` proves that an admissible state term under a valid context evaluates to a non-zero distinguishability level `Value.valid_distinction lvl` within the pilot valuation model. Verified without `sorry`. [Source: `proofs/lean/MpfClosurePilot.lean`].
- **P111 Affect|Effect Structural Inheritance:** [ **CONSTRUCTIVELY_DISCHARGED_SUBSTANTIVE** ] The vacuous legacy string-label lookup table remains pruned, but P111 has been rebuilt in Lean as valuation-linked `AEInheritance`. For non-failing terms, `theorem P111_affect_effect_inheritance` constructs an inheritance record whose Affect side carries the active admissibility floor and whose Effect side carries the realized valuation level. `theorem P111_inheritance_boundary` proves failed terms collapse to no inheritance record (`none`) via L117 boundary behavior. This is an internal pilot result relative to the encoded `Fails` / `valuation` definitions, not an unrestricted claim about external Affect|Effect structure. [Source: `proofs/lean/MpfClosurePilot.lean`].
- **L118 & P112 Context-Gated Operator Algebra:** [ **CONSTRUCTIVELY_DISCHARGED_SUBSTANTIVE** ] Lean 4 proof pilot (`proofs/lean/MpfClosurePilot.lean`) constructs coupling projection operator `tensor (c : Context) (A B : Projection)` as Context-gated with degenerate fallback (`empty_projection`). `theorem L118_operator_algebra` proves conditionally on `ProjectionClosed c` using `if_pos h_closed`; `theorem P112_projection_intersection_specialization` delegates to L118 under the same closure hypothesis. `theorem tensor_unclosed_boundary` provides an explicit falsifiability check demonstrating collapse to `empty_projection` for unclosed contexts (`if_neg h_not`). [Source: `proofs/lean/MpfClosurePilot.lean`].
...
| **OPEN_BRIDGE_003** | Cross-Domain Continuation Trace | **PROVISIONALLY_FORMALIZED_PENDING_VALIDATION** | **THEORETICAL_ONLY** |
...
- **OPEN_BRIDGE_003 (Cross-Domain Continuation Trace):** Registry-synchronized as provisionally formalized pending validation. The provisional carrier is unresolved continuation $UC(C_i;D_i,R_i,O_i,B_i)$ and the scoped handoff operator is $H_{adm}(UC_i,\tau_{i\to j}) \to UC_j \mid \bot_C^{type} \mid \bot_C^{adm} \mid \bot_C^{trace}$. A handoff is lawful only when domain-transition typing, admissibility context, trace invariants, and orientation-reference continuity are declared and preserved. It does not support subjective continuity, consciousness persistence, observer persistence, primitive time, empirical continuity, or trans-domain identity claims.
...
- **OPEN_BRIDGE_003 Validation Obligations:** construct a positive typed handoff example across declared projection domains; construct a type-failure counterexample with no declared $\tau_{i\to j}$; construct a trace-failure counterexample where residue or orientation-reference invariants are lost; audit compatibility with typed-continuation-domain rules; determine whether $H_{adm}$ composes associatively across multiple projection boundaries.
...
- **OPEN_BRIDGE_001:** Reformulated 2026-06-17 as a **Topological Selector** bridge. The live bridge and theorem-status registries now reconcile this node as **SUPPORTED / STRUCTURAL_ONLY**. This support is scoped to orientation coherence conditioning admissible knot-class selection; it is not direct closure support and does not auto-promote downstream theorem or application projections.
- **OPEN_BRIDGE_001:** Run 08 dated 2026-06-16, reviewed 2026-06-17, showed coherent orientation narrowing admissible knot-class variance under the tested PDE + TDA stack. The proof-obligation registry records PO_001 through PO_005 as satisfied; this supersedes the older provisional bridge-status handling while preserving downstream review locks and claim caps.
...
- **`Propagation_Position`** (`propagation_position` / `pi_P(x, phi)`): Provisional RT-native positional bridge quantity defined as the organizational position of an admissible continuation within the bounded symmetry gradient relative to declared `phi` and distinction density `rho_D`. It requires `RATE_TYPE_ELIGIBLE(x, phi)` and defined `rho_D(x, phi)`, is available only in the coupled interior, and is not identical to speed, probability, entropy, statistics, or any physical propagation constant. [Source: `PATCH_RT_METRIC_BRIDGE_PROPAGATION_POSITION_MINIMAL_DEFINITION_005`].
...
- **`Typed_Projection_Transition_Rules`** (`A|E ->p Π_D(A|E)`, `Π_D(A|E) := D(*|*)`): Governed typed transition rules that move the primitive affect/effect condition into the projection layer only through explicit invocation of `Π_D`. Direct collapse from `A|E` to `D(*|*)` is forbidden. The route supports evaluation of a declared typed representability predicate without establishing unrestricted semantic preservation or exhausting the primitive condition. [Source: `PATCH_PI_RT_CALCULUS_020`; `0016_d_representable_distinction_typed_scope_formalization`].
...
- **Notebook 16 lexicon debt discharge (`CHG_LEXICON_NOTEBOOK16_DEBT_DISCHARGE_001`)**: `novel_organization_rate`, `projection_loss_table`, `projection_driven_degrees_of_freedom`, `organizational_degree_of_freedom`, and `aspect_taxonomy` are no longer `GAP_OPEN`. They are registered as `C1_DEFINED_PROVISIONAL`; measured terms carry L1 bounded single-campaign support, while architecture and taxonomy terms remain L0 candidates. No L2 promotion, theorem, universal, ontology-expansion, or external-physical claim is licensed. [Source: registry/governance/patches/CHG_LEXICON_NOTEBOOK16_DEBT_DISCHARGE_001.json]
- **Lexicon debt reduction (`LEXICON_DEBT_REDUCTION_001`)**: The remaining 35 queue terms introduced by the staged induction, Colab intake, Notebook 13–15 topology work, and execution-governance repairs are now `C1_DEFINED_PROVISIONAL` with explicit bounded definitions and resolution notes. This reduces classification debt only; no term is promoted to L2, and no empirical, theorem, universal, or external-physical claim is licensed. [Source: `registry/lexicon_gap_queue.json`]
- **Runtime core-term induction (2026-07-26)**: `process`, `continuation`, `mismatch`, `R`, `Pi_A`, `gate`, `admissibility`, `NavT`, `Xi`, `causal_sphere_of_influence`, and `Constraint Geometry` were added to `registry/lexicon_gap_queue.json` as `GAP_OPEN` after the governance runtime identified them as absent from the exact validation-registry key set. They remain undefined, unvalidated, and unpromoted; this induction does not clear the runtime block or authorize canonical promotion. [Source: `registry/runtime_governance_manifest.json`; `registry/lexicon_validation_registry.json`; `registry/lexicon_gap_queue.json`]
- **Runtime core-term L0 registration (2026-07-26)**: Following human approval of bounded review package `CAMPAIGN_LEXICON_RUNTIME_CORE_TERM_REVIEW_20260726_001`, the same 11 exact keys were registered under `registry/lexicon_validation_registry.json` at `L0` provisional status with candidate roles and no validation evidence. The runtime exact-key check now passes, but the terms remain `GAP_OPEN`, `C0_UNDEFINED`, unvalidated, and unpromoted; this is registry presence synchronization, not L2 validation or canonical promotion. [Source: `departments/analysis/campaigns/CAMPAIGN_LEXICON_RUNTIME_CORE_TERM_REVIEW_20260726_001/lexicon_review_package.json`; `registry/lexicon_validation_registry.json`]
...
- **`GOV-DB-FRESH-001`**: DB snapshot freshness is a mandatory closeout gate. After final source edits, `python scripts/db/snapshot_registries.py` must succeed and the freshness gate must be rerun; `stale`, `unknown`, `unavailable`, or failed status leaves the governed task open. [Source: `governance/live/governance_constitution.json`; `docs/governance/GLOBAL_VALIDATION_ROUTINE.md`]
...
- **D semantics package:** `L131` remains conditional, `P125` remains scaffold-pending-discharge for the broader D projection obligation set, and `P126` records the bounded human-reviewed type-level closure for `OBL-D-001C`. The 2026-07-30 human-approved P127/P128 disposition records `OBL-D-001D` and `OBL-D-001E` as `DISCHARGED_BOUNDED` within finite declared scopes. `DEBT_D_SEMANTICS_PROOF_001` remains active for universal preservation, universal threshold derivation, and theorem promotion; the claim ceiling remains `C1_DEFINED_PROVISIONAL`.
...
- **`NB13_TOPOLOGICAL_INVARIANTS_CONTINUATION_GEOMETRY_001`**: Notebook 13 is specified, not executed, as a topology-only test of whether reachability lattice, SCC condensation DAG, basin decomposition, articulation hierarchy, irreversible separation depth, merge depth, partial-order width, dominance tree, and closure lattice invariants classify Notebook 12 reconvergence geometry and J. Its current status is `C1_SPECIFICATION_ONLY`.
- **`NB13_TOPOLOGICAL_INVARIANTS_CONTINUATION_GEOMETRY_002`**: Notebook 13 v2 supersedes the v1 Colab execution scaffold after user-reported path, order, runtime-reset, wrong-column mapping, crash, and lost-data failures. V2 keeps the same research question but uses one bootstrap cell, actual Notebook 12 column mapping, chunk checkpoints, and resumable aggregation. Its current status is `C1_SPECIFICATION_ONLY`.
- **`NB13_TOPOLOGICAL_INVARIANTS_RESULTS_001`**: Notebook 13 v2 result archive is inducted as `C2_BOUNDED_NOTEBOOK_OUTPUT_WITH_LIMITATIONS`. Direct observations: 13,824 loaded rows, 13,824 invariant rows, zero skips, dummy balanced accuracy 0.25, and topology-feature balanced accuracy 1.0. Limitation: direct topology proxies for the label definition are present, graph-size-only and grouped controls are missing, and approved-tool replication remains pending.
- **`NB14_GENERIC_TOPOLOGY_FEATURE_ABLATION_001`**: Notebook 14 is specified, not executed, as a follow-up ablation test. It prohibits `root_branch_pair_count`, `irreversible_separated_pair_count`, `has_irreversible_separation`, and merge-depth quantities from generic topology classification, then tests SCC, condensation, articulation, bridge, partial-order, reachability, basin, diameter/depth, dominance-tree, and graph-size feature families by removal ablation. Its current status is `C1_SPECIFICATION_ONLY`.
- **`NB14_GENERIC_TOPOLOGY_FEATURE_ABLATION_RESULTS_001`**: Notebook 14 result archive is inducted as `C2_BOUNDED_NOTEBOOK_OUTPUT_WITH_LIMITATIONS`. Direct observations: all-generic balanced accuracy 0.931843 after forbidden proxy exclusion, graph-size-only balanced accuracy 0.710971, forbidden-proxy-only diagnostic accuracy 1.0, and articulation-hierarchy removal produced the largest observed drop, about 0.117538. Limitations: `manifest.json` is present beside the zip but not inside it, grouped evaluation is missing, correlated split leakage remains possible, graph diameter was not present as a distinct feature, and approved-tool replication remains pending.
- **`NB15_ARTICULATION_CONTINUATION_GEOMETRY_THEORY_001`**: Notebook 15 is specified, not executed, as a mathematical articulation notebook. It shifts the inquiry from "articulation predicts geometry" toward theorem-candidate analysis asking whether articulation structure is mathematically linked to continuation geometry. Its current status is `C1_SPECIFICATION_ONLY`; no theorem, causality, implementation-independent, or external physical claim is licensed.
- **`NB15_ARTICULATION_CONTINUATION_GEOMETRY_THEORY_RESULTS_001`**: Notebook 15 result archive is inducted as `C2_THEOREM_CANDIDATE_EVIDENCE_WITH_LIMITATIONS`. Direct observations: 13,824 rows loaded, 5 theorem candidates evaluated, 4 supported within the enumerated domain, and 1 refuted. Supported in-domain candidates include obstruction implying articulation presence, defined articulation depth, multiple terminal basins, and nontrivial closure lattice. The claim that universal reconvergence excludes articulation is refuted by 6,022 counterexamples. No formal theorem or causal claim is licensed.
- **`MPF_SIM_PROJECTION_DOF_ISOLATION_001`**: Notebook 16 is specified as a bounded symbolic organizational-DoF campaign varying admissible slot count while holding the primitive binding law fixed. It measures lawful organization growth, closure, entropy, projection diversity, primitive preservation, branching, symmetry classification, novel organization rate, and projection-loss behavior. Its current status is `C1_SPECIFICATION_ONLY`.
- **`MPF_SIM_PROJECTION_DOF_ISOLATION_RESULTS_001`**: Notebook 16 result archive is inducted as `C2_BOUNDED_NOTEBOOK_OUTPUT_WITH_LIMITATIONS`. Direct observations: lawful organization counts rise from 4 at DoF 1 to 188 at DoF 6, primitive preservation remains 1.0 at every tested DoF, nonzero novel-organization rates appear from DoF 2 upward, and lower-DoF projection is frequently lossy (`lossless_rate = 0.2996031746031746`). Limitations: the archive manifest self-state is stale and still says `NOT_EXECUTED_IN_SOURCE_NOTEBOOK`, novelty depends on the declared projection operator and bounded sweep, the explored domain is limited to `DoF <= 6` and `max_depth = 4`, and approved-tool replication remains pending.
- **`MPF_SIM_PROJECTION_DOF_MEANINGFUL_001`**: The meaningful Notebook 16 follow-on source is classified separately as `C1_NOTEBOOK_PROVENANCE` and nested in the existing Notebook 16 design bundle. Its executed archive is inducted as `C2_BOUNDED_NOTEBOOK_OUTPUT_WITH_LIMITATIONS`: 527 rows parsed with zero parse failures, lawful organization counts rise from 4 at DoF 1 to 188 at DoF 6, normalized enrichment has reported Spearman rho `0.4123` with bootstrap 95% interval `[0.3344, 0.4862]`, and novel catalog classes occur from DoF 2 onward. Limitations: no distinct immutable pre-execution specification was supplied for this follow-on, the operationalization is an ordered rooted symbolic expression graph, the domain is bounded, and approved-tool replication remains pending.
- **`MPF_SIM_ORGANIZATION_RESOLUTION_CALCULUS_001`**: Notebook 17 is nested in `notebook_17_organization_resolution_calculus/` and its archive is inducted as `C2_BOUNDED_NOTEBOOK_OUTPUT_WITH_LIMITATIONS`. Direct observations: 527 organizations, 2,016 projection rows, 15,810 perturbation rows, reference variation for 515 of 527 organizations, and a best candidate `O_MAX` with projection monotonicity rate `0.82738` but counterexamples. The result supports only bounded operational organization-candidate and reference-dependent resolution observations. Limitations: no distinct immutable pre-execution specification, primitive preservation not testable, perturbation-level delta data missing, bounded symbolic domain, and approved-tool replication pending.
- **`NB18_DOMAIN_PROJECTION_RESIDUE_RESULTS_001`**: Notebook 18 is homed in `notebook_18_domain_projection_primitive_hierarchy/` and its archive is inducted as `C2_BOUNDED_NOTEBOOK_OUTPUT_WITH_LIMITATIONS`. Direct observations: seed `180018`, 1,217,346 projection records, and 11,762 strong projections; the information, structural, and reference residue criteria have zero-residue counterexamples, while the typed criterion is not falsified in the tested model class but is partly definition-dependent. The post-execution reconstructed specification does not satisfy the immutable pre-execution gate. No universal residue law, unrestricted primitive hierarchy, external physical claim, or C5/C6 promotion is licensed.
- **`NB19_EXCLUSION_GENERATES_RESIDUE_RESULTS_001`**: Notebook 19 is homed in `notebook_19_exclusion_generates_residue/` and its archive is inducted as `C2_BOUNDED_NOTEBOOK_OUTPUT_WITH_LIMITATIONS`. Direct observations: 1,217,346 total records and 11,762 strong records; strong information and structural exclusion/residue criteria falsify both sufficiency and necessity. Strong total residue has no exclusion-without-residue cases, but 706 residue-without-exclusion cases falsify necessity and therefore the full biconditional. The specification is reconstructed after execution; no universal exclusion law or causal claim is licensed.
- **`NB20_DISTINCTION_CONSERVATION_EXCLUSION_DYNAMICS_RESULTS_001`**: Notebook 20 is homed in `notebook_20_distinction_conservation_exclusion_dynamics/` and its archive is inducted as `C2_BOUNDED_NOTEBOOK_OUTPUT_WITH_LIMITATIONS`. Direct observations: 4,813,425 projections, 25,478,208 distinction instances, zero conservation failures, and 478 matched-exclusion counterexamples to the strong residue-determination form. Conservation remains bounded evidence because the partition is operational and may be definition-dependent; no universal conservation or causal residue law is licensed.
- **`MPF_D_SEMANTICS_PROOF_PACKAGE_001`**: A new additive mathematics package defines `Eval_D,C` as a partial context-indexed evaluation relation and records the typed route `A|E ->p Pi_D(A|E) -> D(*|*)`. `L131` is conditional, `P125` remains `scaffold_pending_discharge` for the broader D package, and `P126` records the bounded human-reviewed type-preservation closure for `OBL-D-001C`. `OBL-D-001A` is discharged, `OBL-D-001B` is discharged within bounded scope, `OBL-D-001C` is discharged within bounded type-level scope, and `OBL-D-001D/E` are discharged only within the finite declared P127/P128 scopes. The package remains `C1_DEFINED_PROVISIONAL` and does not establish a theorem, injectivity, reversibility, complete information preservation, semantic preservation outside the tested scope, or universal operator semantics.
- **`OBL-D-001A` discharged contract**: The additive note `0011_d_eval_domain_codomain_contract_candidate.md` makes `Participation_C`, `DefinedResult_D,C`, `EvalResult_D,C`, and three explicit undefined-result reasons visible. Its five typed fixtures passed the bounded checker and received human approval. `OBL-D-001A` is discharged; no theorem promotion follows.
- **`OBL-D-001B` threshold candidate**: The additive note `0012_epsilon_a_context_threshold_candidate.md` binds `epsilon_a,C` to the declared context's materialized admissibility threshold. The threshold is positive and context-indexed, not a global constant and not silently derived during evaluation. Bounded fixtures are recorded, and the later fixed-seed sensitivity reconstruction plus human review yielded a `DISCHARGED_BOUNDED` certificate within the declared finite grid. This remains bounded `C1_DEFINED_PROVISIONAL` support only; universal threshold derivation remains blocked.
- **`OBL-D-001C` typed preservation candidate**: The additive note `0013_d_typed_projection_preservation_candidate.md` derives type-level preservation from the explicit `Pi_D,C` signature and `PATCH_PI_RT_CALCULUS_020`. The additive proof `P126_d_typed_projection_type_preservation_scope_closure.md` closes the currently supported type-level consequence only. Wrong codomains, direct substitution, and untyped sources are rejected, while semantic representable-distinction preservation remains outside this obligation's discharged scope. The bounded fixture check, independent review, and explicit human review are complete, and the obligation is discharged within bounded type-level scope.
...
- **`OBL-D-001B/D/E` projection semantics status**: B now has a bounded completion certificate after independent reconstruction of all 108 Notebook 21 rows, exact archive matching, deterministic replay, and 21 threshold-sensitive cases across three contexts and three fixed seeds. The bounded broader D search found 594 matched-image counterexamples to projection-only representability, while the E sweep classified all five required fixtures and 48 finite combinations without rule violations. B remains limited to the declared finite grid; C/D review and E's dependent promotion-gate review remain pending. Outcome labels and non-empty residue are not used as definitions.
...
**Directly observed/defined:** The math-program validator completed 267 cataloged checks with no failed check and a warning-level aggregate. The Lean 4 pilot compiles with no `sorry`; the governed checker reports 13 target theorems, while the source contains 21 theorem declarations when six constructor/regression guards and two additional boundary declarations are included. Notebook 13-20 outputs remain C2 bounded computational or theorem-candidate evidence with recorded limitations, including the original Notebook 16 bounded symbolic organizational-DoF archive with lawful-count growth, primitive-preservation rate 1.0 across tested DoF, nonzero novelty from DoF 2 upward, and a stale internal execution-state manifest; the meaningful follow-on archive with a positive bounded enrichment trend and an explicit missing-distinct-specification blocker; Notebook 17 with reference-dependent resolution, projection-monotonicity counterexamples, and untestable primitive-preservation coverage; Notebook 18 with bounded counterexamples to three residue criteria and typed non-falsification that is partly definition-dependent; Notebook 19 with bounded counterexamples to exclusion/residue sufficiency and necessity under information and structural criteria, plus total-residue sufficiency that does not extend to necessity; and Notebook 20 with bounded conservation observations, zero observed conservation failures in the tested finite model class, and 478 matched-exclusion counterexamples to the stronger residue-determined claim.
**Formal D package status:** `L131`, `P125`, and `P126` are additive, local-scope artifacts for the governed semantics of `D` and `Pi_D`. P127 and P128 carry bounded human-approved dispositions for `OBL-D-001D` and `OBL-D-001E`; they are not unrestricted theorem proofs. The open debt item is `DEBT_D_SEMANTICS_PROOF_001` at the universal/theorem-promotion layer. `OBL-D-001A` is discharged, `OBL-D-001B` is discharged within bounded scope, `OBL-D-001C` is discharged within bounded type-level scope, and `OBL-D-001D/E` are discharged only within their finite declared bounded scopes.
**Notebook 20 evidence status:** Notebook 20 adds bounded conservation and matched-exclusion counterexample evidence to the Colab sequence. It does not discharge the D semantics obligations or elevate the conservation identity beyond the tested finite model class.
...
**Status: GOVERNED PROVISIONAL. NOT CANONICAL. OBL-D-001D DISCHARGED_BOUNDED.**
...
The canonical soundness boundary remains unchanged: successful `project_w` output may yield `TypedWitness_C` through `AX-REALIZATION-WITNESS`; that axiom does not establish project_w definedness. The relationship between the candidate predicate and `Dom(project_w)` remains unresolved outside the finite declared P127 scope. No universal theorem, axiom, totality claim, injectivity claim, reversibility claim, or complete information-preservation claim is introduced. `OBL-D-001D` is `DISCHARGED_BOUNDED` only within the approved finite scope and reopens on counterexample or premise failure.
...
| :--- | :--- | :--- | :--- |
| **MPF-CORE-V1** | [MPF-CORE: MINIMAL WORKING FORMALISM](D:/projects/acellorator/docs/MPF_Core_Formalism_v1.md) | v1.0 | Core formalism source used by this textbook; live authority routing must classify it before treating it as an independent current SSOT. |
| **SINGULARITY-REBOUND-001** | [The Singularity Rebound Mechanism](results/2026-05-22_run02_Singularity_Rebound/paper.md) | Legacy paper metadata: C5/L3; active validation surface: L2/partial | Evidence for a model-scoped operational distinguishability floor and recursive trigger interpretation; active charter gates control any current claim level. |
...
<!-- BEGIN AUTO-SYNCHRONIZED GOVERNED RESEARCH STATE -->

This section is generated from canonical induction, queue, intake, and representation records. It is a projection, not an authority source.
...
- **### Summary Generic RT and the QMGR** (`FILE_SUMMARY_GENERIC_RT_QMGR_20260801_001`): Preserved Analysis Intake capture.
  - Capture: `PRESERVED_LITERAL`; Review: `NOT_REVIEWED`; Promotion: `HOLD_C1`; Induction: `NOT_QUEUED`
  - Queue: `NOT_PRESENT`; Registry: `NOT_BOUND`
  - Source: `departments/analysis_intake/chat_captures/### Summary Generic RT and the QMGR.txt`; receipt: `departments/analysis_intake/chat_captures/FILE_SUMMARY_GENERIC_RT_QMGR_20260801_001_receipt.json`; SHA-256: `69CCCEADAC89A5D3498950F75927184761ACD052CCBA3024D18D6FCC3DD7E9BE`

- **RT_CONTINUITY_DISCRETENESS_DOMAIN_HYPOTHESIS_INDUCTION_20260726_001_APPEND_001** (`RT_CONTINUITY_DISCRETENESS_DOMAIN_HYPOTHESIS_INDUCTION_20260726_001_APPEND_001`): Preserved Analysis Intake capture.
  - Capture: `APPENDED_WITH_PARENT_IMMUTABLE`; Review: `APPROVED_FOR_INDUCTION_SCOPE_ONLY`; Promotion: `HOLD_C1`; Induction: `NOT_QUEUED`
  - Queue: `NOT_PRESENT`; Registry: `NOT_BOUND`
  - Source: `departments/analysis_intake/chat_captures/RT_CONTINUITY_DISCRETENESS_DOMAIN_HYPOTHESIS_INDUCTION_20260726_001_append_001.json`; receipt: `NOT_RECORDED`; SHA-256: `03a4ad600a086c5a13984aca5e3595f7ff3559f2494c943538f7d18386d76c40`

- **RT_GR_BOUNDED_SYMMETRY_EXTENSION_BRIDGE_INDUCTION_20260727_001** (`RT_GR_BOUNDED_SYMMETRY_EXTENSION_BRIDGE_INDUCTION_20260727_001`): First-contact conceptual RT-to-GR bridge induction. No semantic review, canonicalization, theorem promotion, GR validity claim, or model campaign execution performed.
  - Capture: `PRESERVED_LITERAL`; Review: `NOT_REVIEWED`; Promotion: `HOLD_C1`; Induction: `NOT_QUEUED`
  - Queue: `NOT_PRESENT`; Registry: `NOT_BOUND`
  - Source: `departments/analysis_intake/chat_captures/RT_GR_BOUNDED_SYMMETRY_EXTENSION_BRIDGE_INDUCTION_20260727_001.json`; receipt: `NOT_RECORDED`; SHA-256: `D95CFA9D3041ECF7FF4FE16A86C38B8DDF9EBF8EF4F55F91709CB7815D46024F`

- **RT_INDUCTION_CONTINUITY_DISCRETENESS_SYMMETRY_PROJECTION_20260726_001** (`RT_INDUCTION_CONTINUITY_DISCRETENESS_SYMMETRY_PROJECTION_20260726_001`): Preserved Analysis Intake capture.
  - Capture: `PRESERVED_PROVISIONAL`; Review: `NOT_REVIEWED`; Promotion: `HOLD_C1`; Induction: `NOT_QUEUED`
  - Queue: `NOT_PRESENT`; Registry: `NOT_BOUND`
  - Source: `departments/analysis_intake/chat_captures/RT_INDUCTION_CONTINUITY_DISCRETENESS_SYMMETRY_PROJECTION_20260726_001.json`; receipt: `NOT_RECORDED`; SHA-256: `bb6618c0e91da2c7881bdedf714f9e2cb2cc8f3f5e6e8813e04e539793d70653`

- **RT_INDUCTION_DISCRETE_CONDITION_CONTINUOUS_PATH_20260726_002** (`RT_INDUCTION_DISCRETE_CONDITION_CONTINUOUS_PATH_20260726_002`): Preserved Analysis Intake capture.
  - Capture: `PRESERVED_PROVISIONAL`; Review: `NOT_REVIEWED`; Promotion: `HOLD_C1`; Induction: `NOT_QUEUED`
  - Queue: `NOT_PRESENT`; Registry: `NOT_BOUND`
...
- **Orientation, Distinction Density, and Triad Progression Session Intake** (`MPF_IND_ORIENTATION_DENSITY_TRIAD_SESSION_001`): Provisional session intake from chat session 2026-07-20. Open items routed to MPF_ORIENTATION_STALENESS_AUDIT_001 per governance note.
  - Capture: `NOT_RECORDED`; Review: `NOT_RECORDED`; Promotion: `NOT_RECORDED`; Induction: `NOT_QUEUED`
  - Queue: `NOT_PRESENT`; Registry: `C1_DEFINED_PROVISIONAL`
...
## Archived or Explicitly Excluded Research Notes

- **Notebook 23 D/E Held-Out Stress Source** (`MPF_SIM_D_E_HELD_OUT_STRESS_001`): User-directed exclusion freeze. Retained only as quarantined provenance; excluded from claim registries, textbook, publication, lexicon, downstream reports, and promotion. See registry/governance/notebook_23_exclusion_freeze_registry.json.
  - Capture: `NOT_RECORDED`; Review: `NOT_RECORDED`; Promotion: `NOT_RECORDED`; Induction: `NOT_QUEUED`
  - Queue: `excluded_frozen`; Registry: `EXCLUDED_FROZEN`
  - Source: `departments/colab/notebook_designs/notebook_23_d_e_held_out_stress/RT_Notebook_23_D_E_Held_Out_Stress.ipynb`; receipt: `NOT_RECORDED`; SHA-256: `72cc833431afdb18fdb283b4c07ec560cf99fafd5d10c9f15ea65a32713c1c7a`

- **Notebook 23 D/E Held-Out Stress Result Archive** (`MPF_SIM_D_E_HELD_OUT_STRESS_RESULTS_001`): User-directed exclusion freeze. Archive is not evidence, not a textbook source, and not eligible for induction or promotion. Retained only as quarantined provenance. See registry/governance/notebook_23_exclusion_freeze_registry.json.
  - Capture: `NOT_RECORDED`; Review: `NOT_RECORDED`; Promotion: `NOT_RECORDED`; Induction: `NOT_QUEUED`
  - Queue: `excluded_frozen`; Registry: `EXCLUDED_FROZEN`
  - Source: `departments/colab/results/MPF_SIM_D_E_HELD_OUT_STRESS_001_RESULTS.zip`; receipt: `NOT_RECORDED`; SHA-256: `9274a205bda40886662e59f5e27d512c4805cdffa7a2516c1528f670546bc649`

- **Notebook 16 Projection-Driven Organizational DoF Meaningful Follow-on Source** (`MPF_SIM_PROJECTION_DOF_MEANINGFUL_001`): Source notebook is classified separately from the executed archive. It has no distinct immutable pre-execution experiment specification; the earlier isolation specification is not treated as a substitute.
  - Capture: `NOT_RECORDED`; Review: `NOT_RECORDED`; Promotion: `NOT_RECORDED`; Induction: `NOT_QUEUED`
  - Queue: `excluded_frozen`; Registry: `EXCLUDED_FROZEN`
  - Source: `departments/colab/notebook_designs/notebook_16_projection_dof_isolation/RT_Notebook_16_Projection_DoF_Meaningful_Experiment.ipynb`; receipt: `NOT_RECORDED`; SHA-256: `0cdee99aeeb5bb5a5355212e8c50c30a41136d0e7f6589d03c4d66a7ad5c8101`

<!-- END AUTO-SYNCHRONIZED GOVERNED RESEARCH STATE -->

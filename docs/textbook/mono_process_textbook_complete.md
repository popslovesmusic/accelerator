...
- **Symmetry-Condition Relation and Dominant-Domain Projection:** [ **C1_DEFINED_PROVISIONAL** ] `RT_SYMMETRY_CONDITION_RELATION_CORRECTION_001` corrects the relation signature to `| : BoundedSymmetry × UnboundedSymmetry -> SymmetryCondition`, with `<S>|>S< = S`; `(*|*)` is the role-sensitive notation for `S` as distinction-permitting. AX-R01 and AX-R02 are provisionally formalized; AX-R03 is operator-local `INAPPLICABLE`, AX-R04 permits only canonical self-reference and governed alias normalization, AX-R05 self-composition is `INAPPLICABLE`, AX-R06 records operator-local `RESOLVED_NOT_CLOSED`, and AX-R07 provisionally formalizes validation-only malformed-construction rejection. Whole-RT identity/composition and SymmetryCondition-to-operand projection remain separate open obligations; AX-R08 through AX-R12, selection, executable semantics, and external ontology remain unestablished. Superseded constructor/output-type entries are retained as aliases in `registry/lexicon_gap_queue.json`.

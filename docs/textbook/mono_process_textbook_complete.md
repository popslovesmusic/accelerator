...
- **Vertical Bar Operator ($\mid$):** [ **C1_DEFINED_TO_C2_TESTABLE_CANDIDATE / CAMPAIGN_SCAFFOLD_READY** ] Formally fixing the meaning of "grouped closure participation" in triadic closure. A candidate formal definition exists in `formal_object_registry [OBJ-grouped-bar-closure]` and `operator_registry.json` (status: `MUST_DEFINE_BEFORE_ORG_A_CLOSURE`). Group closure candidate symbol: $\text{Adm}_{|}^{G}$. Associativity and permutability default to NON-PROVEN until tested. Campaign scaffold ready for testing. [Patch: MPF_VERTICAL_BAR_INDUCTION_001, MPF_VERTICAL_BAR_CAMPAIGN_SCAFFOLD_PATCH_001; Campaign: MPF_BAR_OPERATOR_VALIDATION_001]
...
- **MPF_BAR_OPERATOR_VALIDATION_001:** Test | as a distinction-preserving comparison operator under 4 controls and 5 metrics; campaign scaffold ready. [Patch: MPF_VERTICAL_BAR_CAMPAIGN_SCAFFOLD_PATCH_001]

...
**Formal D package status:** `L131`, `P125`, and `P126` are additive, local-scope artifacts for the governed semantics of `D` and `Pi_D`. They are not included among the governed checker’s formally discharged theorem targets. The open debt item is `DEBT_D_SEMANTICS_PROOF_001`, with live next-step obligations `OBL-D-001C` through `OBL-D-001E`; `OBL-D-001A` is discharged and `OBL-D-001B` is discharged within bounded scope.

...
- **Directed Relational Distinction ($D$):** [ **C1_DEFINED_PROVISIONAL** ] The notation and typed projection route are synchronized, but evaluation domain/codomain, context thresholds, preservation, and non-collapse semantics remain open obligations under `DEBT_D_SEMANTICS_PROOF_001`.
- **Local Admissibility Floor ($\epsilon$):** [ **C1_DEFINED** ]
...
- **D semantics package:** `L131` is conditional and `P125` is scaffold-pending-discharge for the local semantics of `D` and `Pi_D`. `DEBT_D_SEMANTICS_PROOF_001` remains open with `OBL-D-001A` through `OBL-D-001E`; Appendix F synchronization discharges documentation drift only, not the substantive proof debt.
...
- **`MPF_D_SEMANTICS_PROOF_PACKAGE_001`**: A new additive mathematics package defines `Eval_D,C` as a partial context-indexed evaluation relation and records the typed route `A|E ->p Pi_D(A|E) -> D(*|*)`. `L131` is conditional and `P125` is `scaffold_pending_discharge`; five obligations remain open for domain/codomain, threshold semantics, type preservation, representable-distinction preservation, and non-collapse. The package remains `C1_DEFINED_PROVISIONAL` and does not establish a theorem, injectivity, reversibility, complete information preservation, or universal operator semantics.
...
- **`OBL-D-001B` threshold candidate**: The additive note `0012_epsilon_a_context_threshold_candidate.md` binds `epsilon_a,C` to the declared context's materialized admissibility threshold. The threshold is positive and context-indexed, not a global constant and not silently derived during evaluation. Bounded fixtures are recorded; sensitivity testing and human review remain pending.
- **`OBL-D-001C` typed preservation candidate**: The additive note `0013_d_typed_projection_preservation_candidate.md` derives type-level preservation from the explicit `Pi_D,C` signature and `PATCH_PI_RT_CALCULUS_020`. It rejects wrong codomains, direct substitution, and untyped sources, while making no claim about injectivity, reversibility, or semantic information preservation. Fixtures and review remain pending.
- **`OBL-D-001D/E` representation and collapse candidates**: The additive notes `0014_d_representable_distinction_candidate.md` and `0015_d_noncollapse_boundary_candidate.md` define representability through typed witnesses and trace compatibility, and define collapse as undefined projection or loss of required representation. The bounded broader search (`d_representable_distinction_broader_search_20260723.json`) found 594 matched-image counterexamples to projection-only representability and label collisions for all tested outcomes. The bounded E sweep (`d_noncollapse_boundary_countermodel_search_20260723.json`) classified all five required fixtures and 48 finite combinations without rule violations. User approval now covers human review for B and E; B still requires fixed-seed sensitivity, and E still requires dependent gate review. Outcome labels and non-empty residue are not used as definitions. C/D review and semantic-preservation wording qualification remain pending.

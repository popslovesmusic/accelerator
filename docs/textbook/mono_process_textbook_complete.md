...
- **Directed Relational Distinction ($D$):** [ **C1_DEFINED_PROVISIONAL** ] The notation and typed projection route are synchronized. Evaluation domain/codomain is discharged, context thresholds carry bounded support, and typed projection preservation is discharged within bounded human-reviewed type-level scope; representable-distinction preservation and non-collapse semantics remain open obligations under `DEBT_D_SEMANTICS_PROOF_001`.
- **Local Admissibility Floor ($\epsilon$):** [ **C1_DEFINED** ]
...
- **D semantics package:** `L131` is conditional, `P125` remains scaffold-pending-discharge for the broader D projection obligation set, and `P126` records the bounded human-reviewed type-level closure for `OBL-D-001C`. `DEBT_D_SEMANTICS_PROOF_001` remains open with live blockers `OBL-D-001D` and `OBL-D-001E`; `OBL-D-001A` is discharged, `OBL-D-001B` is discharged within bounded scope, and `OBL-D-001C` is discharged within bounded type-level scope. Appendix F synchronization discharges documentation drift only, not the substantive proof debt.
...
**Formal D package status:** `L131`, `P125`, and `P126` are additive, local-scope artifacts for the governed semantics of `D` and `Pi_D`. They are not included among the governed checker’s formally discharged theorem targets. The open debt item is `DEBT_D_SEMANTICS_PROOF_001`, with live next-step obligations `OBL-D-001D` and `OBL-D-001E`; `OBL-D-001A` is discharged, `OBL-D-001B` is discharged within bounded scope, and `OBL-D-001C` is discharged within bounded type-level scope.

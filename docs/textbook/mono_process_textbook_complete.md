...
- **`OBL-D-001A` candidate contract**: The additive candidate note `0011_d_eval_domain_codomain_contract_candidate.md` makes `Participation_C`, `DefinedResult_D,C`, `EvalResult_D,C`, and three explicit undefined-result reasons visible. Its fixtures distinguish incomplete context, inadmissibility, projection failure, and pre-domain rejection of untyped input. The candidate remains pending checker execution and human review; `OBL-D-001A` remains open and no promotion follows.

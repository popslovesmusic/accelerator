...
- **Directed Relational Distinction ($D$):** [ **C1_DEFINED_PROVISIONAL** ] The notation and typed projection route are synchronized. Evaluation domain/codomain is discharged, context thresholds carry bounded support, and typed projection preservation now has a scope-corrected type-level closure candidate; representable-distinction preservation and non-collapse semantics remain open obligations under `DEBT_D_SEMANTICS_PROOF_001`.
- **Local Admissibility Floor ($\epsilon$):** [ **C1_DEFINED** ]
...
- **D semantics package:** `L131` is conditional, `P125` remains scaffold-pending-discharge for the broader D projection obligation set, and `P126` now records a bounded type-level closure for `OBL-D-001C`. `DEBT_D_SEMANTICS_PROOF_001` remains open with live blockers `OBL-D-001C` through `OBL-D-001E`; `OBL-D-001A` is discharged and `OBL-D-001B` is discharged within bounded scope. Appendix F synchronization discharges documentation drift only, not the substantive proof debt.
...
- **`MPF_D_SEMANTICS_PROOF_PACKAGE_001`**: A new additive mathematics package defines `Eval_D,C` as a partial context-indexed evaluation relation and records the typed route `A|E ->p Pi_D(A|E) -> D(*|*)`. `L131` is conditional, `P125` remains `scaffold_pending_discharge` for the broader D package, and `P126` records the current bounded type-preservation closure for `OBL-D-001C`. `OBL-D-001A` is discharged, `OBL-D-001B` is discharged within bounded scope, and the live open obligations are `OBL-D-001C`, `OBL-D-001D`, and `OBL-D-001E`. The package remains `C1_DEFINED_PROVISIONAL` and does not establish a theorem, injectivity, reversibility, complete information preservation, semantic representable-distinction preservation, or universal operator semantics.
...
**Formal D package status:** `L131` and `P125` are additive, conditional/scaffold artifacts for the local semantics of `D` and `Pi_D`. They are not included among the governed checker’s formally discharged theorem targets. The open debt item is `DEBT_D_SEMANTICS_PROOF_001`, with next-step obligations `OBL-D-001A` through `OBL-D-001E`.
...
**Formal D package status (Friday, July 24, 2026 sync):** `L131`, `P125`, and `P126` are additive, local-scope artifacts for the governed semantics of `D` and `Pi_D`. They are not included among the governed checker’s formally discharged theorem targets. The open debt item is `DEBT_D_SEMANTICS_PROOF_001`, with live next-step obligations `OBL-D-001C` through `OBL-D-001E`; `OBL-D-001A` is discharged and `OBL-D-001B` is discharged within bounded scope.

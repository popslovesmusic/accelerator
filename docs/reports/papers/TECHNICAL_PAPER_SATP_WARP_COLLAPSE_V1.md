...
- **Warp Lock [PROVISIONAL]:** Sustained anchored-scale opposition ($W(r,t) > W_{opp}$) between the identity and the local orientation basin $\omega_t$.
...
- **Justification:** The claim status is downgraded due to:
  ...
  3. Dependency on unverified lexicon terms (`SATP`, `Warp Lock`, `Phase-Flip`) which remain in the `GAP_OPEN` induction state.

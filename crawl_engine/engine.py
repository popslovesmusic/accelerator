...
OPEN_RELATION_AXIOMS = RELATION_AXIOMS[2:]
...
def run(root, output, focus=None):
    ...
    report["blockers"] = [blocker for blocker in report["blockers"] if blocker.get("blocker_id") not in FORMALIZED_RELATION_AXIOMS]
    for blocker in report["blockers"]:
        if blocker.get("blocker_id") == "BLOCK-PROJECTION-PROPAGATED":
            blocker["blocking_dependencies"] = OPEN_RELATION_AXIOMS
    report["delta"]["added_axioms"] = ["AX-R02_TYPE_COMPATIBILITY"]
    report["delta"]["resolved_obligations"] = ["AX-R02_TYPE_COMPATIBILITY"]

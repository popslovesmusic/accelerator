...
def run(root, output, focus=None):
    ...
    report["object_inventory"]["unresolved"] = [{"object_id": axiom, "reason": "open axiom obligation"} for axiom in OPEN_RELATION_AXIOMS]
    report["object_inventory"]["counts"]["unresolved"] = len(OPEN_RELATION_AXIOMS)
    report["proof_state"]["open_obligations"] = OPEN_RELATION_AXIOMS
    report["blockers"] = [blocker for blocker in report["blockers"] if blocker.get("blocker_id") != "AX-R01_OPERAND_ORDER"]
    for blocker in report["blockers"]:
        if blocker.get("blocker_id") == "BLOCK-PROJECTION-PROPAGATED":
            blocker["blocking_dependencies"] = OPEN_RELATION_AXIOMS
    report["delta"]["added_axioms"] = ["AX-R01_OPERAND_ORDER"]
    report["delta"]["resolved_obligations"] = ["AX-R01_OPERAND_ORDER"]

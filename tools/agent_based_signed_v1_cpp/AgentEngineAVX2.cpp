...
void AgentEngineAVX2::setParams(double kappa, double R_c, double K_phi, double mismatch_rate, double mismatch_phase, double bias_strength, double residue_decay) {
    kappa_ = kappa; R_c_ = R_c; K_phi_ = K_phi; mismatch_rate_ = mismatch_rate; 
    mismatch_phase_ = mismatch_phase; bias_strength_ = bias_strength; residue_decay_ = residue_decay;
}
...
void AgentEngineAVX2::computeDerivatives(const AgentBunchSoA& in, AgentBunchSoA& out) {
    ...
    for (int i = 0; i < static_cast<int>(count_); ++i) {
        ...
        // 4. Mismatch & Residue
        double phase_alignment = std::cos(in.phi[i] - mismatch_phase_);
        double effective_mismatch = mismatch_rate_ * (1.0 + bias_strength_ * phase_alignment);

        double coh = (neighbors > 0) ? (local_coherence / neighbors) : 0.0;
        out.mismatch[i] = effective_mismatch - 0.1 * coh * in.mismatch[i];
...
AgentEngineAVX2::Metrics AgentEngineAVX2::getMetrics() const {
    double sum_x = 0, sum_x2 = 0, sum_re = 0, sum_im = 0, sum_res = 0, sum_mis = 0;
    for (size_t i = 0; i < count_; ++i) {
        sum_x += bunch_->x[i];

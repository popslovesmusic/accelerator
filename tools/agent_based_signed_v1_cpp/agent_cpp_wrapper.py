...
class AgentEngineCPP:
    def __init__(self, agent_count, lib_path=None):
        ...
        self.lib.set_swarm_params.argtypes = [ctypes.c_void_p, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double, ctypes.c_double]
    ...
    def set_params(self, kappa, R_c, K_phi, mismatch_rate, mismatch_phase, bias_strength, residue_decay):
        self.lib.set_swarm_params(self.obj, ctypes.c_double(kappa), ctypes.c_double(R_c), ctypes.c_double(K_phi), 
                                  ctypes.c_double(mismatch_rate), ctypes.c_double(mismatch_phase), 
                                  ctypes.c_double(bias_strength), ctypes.c_double(residue_decay))
    ...
    def get_metrics(self):
        x_mean, x_rms, order_param, res_mean, mis_mean = ctypes.c_double(), ctypes.c_double(), ctypes.c_double(), ctypes.c_double(), ctypes.c_double()
        self.lib.get_swarm_metrics(self.obj, ctypes.byref(x_mean), ctypes.byref(x_rms), ctypes.byref(order_param), ctypes.byref(res_mean), ctypes.byref(mis_mean))
        return {
            "x_mean": x_mean.value,
            "x_rms": x_rms.value,
            "order_parameter": order_param.value,
            "residue_mean": res_mean.value,
            "mismatch_mean": mis_mean.value
        }

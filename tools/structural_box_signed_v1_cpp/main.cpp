...
Diagnostic calculate_diagnostics(size_t nx, const T* epsilon, const T* rho, const T* residue, T activity_threshold) {
    ...
    for (size_t i = 0; i < nx; ++i) {
        ...
        if (std::abs(e) >= static_cast<double>(activity_threshold)) active_count++;
    }

    return {e_max, e_min, r_min, res_max, static_cast<double>(active_count) / nx};
}
...
void run_sim(sycl::queue& q, size_t nx, T dt, T length,
    ...
    if (sequence.is_array() && !sequence.empty()) {
        for (const auto& phase : sequence) {
            if (phase.contains("injection")) {
                const auto& inj = phase["injection"];
                T i_amp = inj.value("amplitude", static_cast<T>(0.32));
                T i_sigma = inj.value("sigma", static_cast<T>(0.08));
                T i_offset = inj.value("offset", static_cast<T>(0.0));
                engine.add_gaussian(0.0, i_amp, i_sigma, i_offset, length);
            }

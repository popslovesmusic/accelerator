...
public:
    ...
    void add_gaussian(T base, T amplitude, T sigma, T offset, T length) {
        const size_t nx = nx_;
        auto eps_ptr = epsilon;
        T dx = length / nx;
        q_.parallel_for(sycl::range<1>(nx), [=](sycl::id<1> i) {
            ...
            T x = static_cast<T>(idx) * dx - static_cast<T>(0.5) * length;
            T r2 = (x - offset) * (x - offset);
            eps_ptr[idx] += amplitude * sycl::exp(-static_cast<T>(0.5) * r2 / (sigma * sigma));
        }).wait();
    }
    ...
    void step(T dt, T dx, 
        ...
        q_.parallel_for(sycl::range<1>(nx), [=](sycl::id<1> i) {
            ...
            T absE = (e > static_cast<T>(0) ? e : -e);
            T absR = (R > static_cast<T>(0) ? R : -R);

            T de = D_epsilon * lapE + a * e - b * e * r - c * e * absE + u * R + s;
            T dr = D_rho * lapRho + alpha * r - beta * absE * r - gamma * r * r - v * absR + h;

...
def validate_formal_restricted_proof_consolidation():
    registry_path = "registry/math/formal_restricted_proof_consolidation_review.json"
    result_path = "validation/results/formal_restricted_proof_consolidation_result.json"
    
    report = {
        "validation_id": "VAL-FPC-CON-VALID-001",
        "status": "pass",
        "checks_passed": [],
    ...
    if "Claiming physical derivation from local proofs." not in forbidden:
        report["status"] = "fail"
        report["governance_violations"].append("forbidden_next_steps_include_physics_claim: FAIL")
    else:
        report["checks_passed"].append("forbidden_next_steps_include_physics_claim")
...
if __name__ == "__main__":
    res = validate_formal_restricted_proof_consolidation()

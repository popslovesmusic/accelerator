...
def validate_mt001_readiness(readiness_reg, theorem_reg, blocker_reg):
    results = {
        "mt001_readiness_validation": {
            "status": "pass",
            "theorem_id": "MT-001",
            "warnings": [],
            "errors": []
    ...
    try:
        ...
        results["mt001_readiness_validation"]["errors"].append(f"Load error: {e}")
        return results

    # Verify target theorem
    if readiness_data["readiness_summary"]["theorem_id"] != "MT-001":
        results["mt001_readiness_validation"]["status"] = "fail"
        results["mt001_readiness_validation"]["errors"].append(f"Registry target mismatch: expected MT-001, found {readiness_data['readiness_summary']['theorem_id']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Validate MT-001 formal-candidate readiness.")
    ...
    res = validate_mt001_readiness(args.readiness, args.theorems, args.blockers)

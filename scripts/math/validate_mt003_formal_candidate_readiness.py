...
def validate_mt003_readiness(readiness_reg, theorem_reg, blocker_reg):
    results = {
        "mt003_readiness_validation": {
            "status": "pass",
            "theorem_id": "MT-003",
            "warnings": [],
            "errors": []
    ...
    try:
        ...
        results["mt003_readiness_validation"]["errors"].append(f"Load error: {e}")
        return results

    if readiness_data["readiness_summary"]["theorem_id"] != "MT-003":
        results["mt003_readiness_validation"]["status"] = "fail"
        results["mt003_readiness_validation"]["errors"].append(f"Registry target mismatch: expected MT-003, found {readiness_data['readiness_summary']['theorem_id']}")
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Validate MT-003 formal-candidate readiness.")
    parser.add_argument("--readiness", default="registry/math/mt003_formal_candidate_readiness_registry.json")
    ...
    res = validate_mt003_readiness(args.readiness, args.theorems, args.blockers)

...
def validate_restricted_proof_candidate_consolidation():
    registry_path = "registry/math/restricted_proof_candidate_consolidation_review.json"
    result_path = "validation/results/restricted_proof_candidate_consolidation_result.json"
    
    report = {
        "validation_id": "VAL-RPC-CON-VALID-001",
        "status": "pass",
        "checks_passed": [],
    ...
    if "Claiming theorem proof from candidate support." not in forbidden:
        report["status"] = "fail"
        report["governance_violations"].append("forbidden_next_steps_include_proof_claim: FAIL")
    else:
        report["checks_passed"].append("forbidden_next_steps_include_proof_claim")
...
if __name__ == "__main__":
    res = validate_restricted_proof_candidate_consolidation()

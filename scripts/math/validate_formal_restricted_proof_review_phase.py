...
def validate_formal_restricted_proof_review_phase():
    registry_path = "registry/math/formal_restricted_proof_review_phase_declaration.json"
    result_path = "validation/results/formal_restricted_proof_review_phase_result.json"
    
    report = {
        "validation_id": "VAL-FPR-VALID-001",
        "status": "pass",
        "checks_passed": [],
    ...
    # 2. series_status_equals_RESTRICTED_FORMAL_PROOF_CONSTRUCTION
    if gov.get("series_status") != "RESTRICTED_FORMAL_PROOF_CONSTRUCTION":
        report["status"] = "fail"
        report["governance_violations"].append(f"series_status_equals_RESTRICTED_FORMAL_PROOF_CONSTRUCTION: FAIL (found {gov.get('series_status')})")
    else:
        report["checks_passed"].append("series_status_equals_RESTRICTED_FORMAL_PROOF_CONSTRUCTION")
    ...
    required_forbidden = [
        ("A local proof attempt justifies global promotion.", "forbidden_claims_include_local_to_global"),
        ("Derivability under assumptions proves framework closure.", "forbidden_claims_include_derivability_proves_closure")
    ]
...
if __name__ == "__main__":
    res = validate_formal_restricted_proof_review_phase()

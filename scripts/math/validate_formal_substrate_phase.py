...
def validate_formal_substrate_phase():
    registry_path = "registry/math/formal_substrate_phase_declaration.json"
    result_path = "validation/results/formal_substrate_phase_result.json"
    
    report = {
        "validation_id": "VAL-FSP-VALID-001",
        "status": "pass",
        "checks_passed": [],
    ...
    # 2. series_status_equals_FORMAL_SUBSTRATE_SCAFFOLD
    if gov.get("series_status") != "FORMAL_SUBSTRATE_SCAFFOLD":
        report["status"] = "fail"
        report["governance_violations"].append(f"series_status_equals_FORMAL_SUBSTRATE_SCAFFOLD: FAIL (found {gov.get('series_status')})")
    else:
        report["checks_passed"].append("series_status_equals_FORMAL_SUBSTRATE_SCAFFOLD")
    ...
    required_forbidden = [
        ("The formal substrate proves the framework is 'real'.", "forbidden_claims_include_reality_proof"),
        ("Substrate definitions derive physical constants.", "forbidden_claims_include_physics_derivation")
    ]
...
if __name__ == "__main__":
    res = validate_formal_substrate_phase()

...
def validate_reconstruction_dynamics_phase():
    registry_path = "registry/math/reconstruction_dynamics_phase_declaration.json"
    result_path = "validation/results/reconstruction_dynamics_phase_result.json"
    
    report = {
        "validation_id": "VAL-RDP-VALID-001",
        "status": "pass",
        "checks_passed": [],
    ...
    # 2. series_status_equals_DYNAMICS_SCAFFOLD_ONLY
    if gov.get("series_status") != "DYNAMICS_SCAFFOLD_ONLY":
        report["status"] = "fail"
        report["governance_violations"].append(f"series_status_equals_DYNAMICS_SCAFFOLD_ONLY: FAIL (found {gov.get('series_status')})")
    else:
        report["checks_passed"].append("series_status_equals_DYNAMICS_SCAFFOLD_ONLY")
    ...
    required_forbidden = [
        ("Reality literally evolves according to reconstruction dynamics.", "forbidden_claims_include_reality_evolution"),
        ("Stability implies truth or ontology identity.", "forbidden_claims_include_stability_identity")
    ]
...
if __name__ == "__main__":
    res = validate_reconstruction_dynamics_phase()

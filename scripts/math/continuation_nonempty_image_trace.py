...
def trace_continuation_nonempty_image(query_id, scaffold_path, theorem_reg):
    try:
    ...
    if query_id != "MT-003" and query_id != scaffold.get("theorem_reference"):
        return {"error": f"Theorem {query_id} not found in MT-003 proof scaffold."}

    trace = {
        "continuation_nonempty_image_trace": {
            ...
            "remaining_gaps": [
                "Formal definition of existence(continuation_event)",
                "Verification of Pi_A image properties",
                "Stability of selection rule results"
            ]
        }
    }
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Trace MT-003 proof dependencies.")
    parser.add_argument("--query", required=True, help="Theorem ID (e.g., MT-003)")
    ...
    res = trace_continuation_nonempty_image(args.query, args.scaffold, args.theorems)

...
def run_relational_asymmetry():
    ...
    kuramoto.omega[1] = 1.2 # High mismatch
    ...
    # Destroy kuramoto engine to free SYCL context before starting metrics
    del kuramoto
    import gc
    gc.collect()

    print("Initializing Metrics Engine...")
    ...
    # 4. Result Synthesis
    # Node 1 (highest omega) should have highest outgoing MI if it's the "driver"
    outgoing_sum = np.sum(mi_matrix, axis=1)
    incoming_sum = np.sum(mi_matrix, axis=0)
    
    # Specialization: Driver = high Out/In ratio, Stabilizer = low Out/In ratio

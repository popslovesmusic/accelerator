...
def run(config_path: str, out_dir: str) -> None:
    ...
    trace_window_steps = int(cdhds_cfg.get("trace_window_steps", 200))
    ...
    for step in range(steps):
        # Continuous evolution of phases
        phi = net.step_phi(phi, dt)

        # Admissible continuation set: candidate recoupling edges (stress below threshold).
        ...
        # Discrete δ-event: topology rewiring step (edge additions/removals).
        net.rewire(phi)
        A_after = net.A
        ...
        # Traceability: any added edge must have been a candidate within the trace window.
...
def main() -> None:
    parser = argparse.ArgumentParser(description="CDHDS wrapper runner for graph_dynamics_sim_v1")

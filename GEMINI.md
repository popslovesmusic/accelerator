...
1. Running simulations across independent engines in `tools/`.
2. Comparing results across model classes.
3. Producing governed, falsifiable technical papers.

**Mandatory Inclusion:** The agent MUST include the specific **model** being tested, cited, or discussed in all reports and writing.
...
- You MAY create new config JSON files for experiments in `configs/`.
...
- **Reference Document:** Refer to `USAGE.md` for operational instructions and standard directory layouts.
...
All terms MUST resolve through the canonical lexicon and compliance authority in `registry/`:

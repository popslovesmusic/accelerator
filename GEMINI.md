...
- **Textbook Synchronization Check:** Before finalizing any governed task, audit `docs/textbook/mono_process_textbook_complete.md` against the current outputs, registries, proof obligations, and claim-gate result. Any stale linked textbook section must be patched or the mismatch must be explicitly reported as unresolved.
...
- **Center-Control Falsification Attack (New - Mandatory):** All high-level framework claims (C4+) must survive the **Center-Control Falsification Attack** (registry/governance/falsification_attack_method.json). This adversarial protocol requires systematic ablation of support mechanisms (orientation, residue, admissibility) before any application-level claim (space_app, matter_app, gravity_app) is promoted. Support is earned through survival against matched-random controls, not merely emergent appearance.
- **Open Bridge Dependency Rule (New - Mandatory):** Any claim downstream of an unresolved mathematical bridge (e.g., OPEN_BRIDGE_001) may not be promoted above the bridge's current claim level. If a bridge is falsified, all dependent claims must enter **REVIEW_LOCK** until rewritten or retired. Support for a bridge propagates only to immediate structural consequences, not to high-level application projections (rule rule_id: OPEN_BRIDGE_DEPENDENCY_RULE_001).
- **Proof Obligation Object Rule (New - Mandatory):** Every formalized bridge (e.g., OPEN_BRIDGE_001) must be converted into an **Auditable Proof Obligation Object** (registry/math/open_bridge_proof_obligation_registry.json). This object defines the formal establishing requirements (PO_001-PO_007), null/alternative hypotheses, mathematical/simulation obligations, and decision rules for support, downgrade, or falsification. No bridge may be marked SUPPORTED without satisfying its proof obligation.
...
### 19.5 Mathematical Object Classes
...
### 19.6 Proof Classification System
...
### 19.7 Contradiction Resolution Protocol
...
### 19.8 Relational Mapping Taxonomy

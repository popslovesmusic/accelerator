...
- **Chat Submission Persistence Rule:** When a proposal is submitted in chat, first save the complete received payload as a recoverable file under `departments/analysis_intake/`, verify its SHA-256, and use that on-disk copy as the induction source. A chat summary, reconstructed subset, or review note is not a substitute for the saved source artifact.

...
void step_2d(SimulationState& state, const Grid& grid, const UpdateParams& params) {
    ...
    for (size_t y = 0; y < ny; ++y) {
        for (size_t x = 0; x < nx; ++x) {
            ...
            float lap_eps = compute_laplacian_2d(state.epsilon, grid, x, y);
            float grad_eps_x, grad_eps_y;
            compute_gradient_2d(state.epsilon, grid, x, y, grad_eps_x, grad_eps_y);
            ...
            float grad_R_x, grad_R_y;
            compute_gradient_2d(state.R, grid, x, y, grad_R_x, grad_R_y);
            ...
            if (params.falsification_mode == "FV_006_noise_injection") {
                ...
                float noise = std::sin(x * 12.9898f + y * 78.233f + params.step_current * 37.719f) * 43758.5453f;
            ...
            float norm = std::sqrt(ix*ix + iy*iy);
            if (norm > 1e-6f) {
            ...
            // 5. Rho Update
            float aligned_flow = state.I.x[idx] * grad_eps_x + state.I.y[idx] * grad_eps_y;

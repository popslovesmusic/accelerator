...
import numpy as np
import time
from field_engine import SATPHiggsEngine3D
...
def run_simulation(config_path, output_dir):
    ...
    engine = SATPHiggsEngine3D(nx, ny, nz)
    engine.initialize_vacuum(h_vev)
    
    # Spherical Gaussian bump perturbation in phi matching C++ benchmark
    mid_x, mid_y, mid_z = nx // 2, ny // 2, nz // 2
    sigma = 0.05 * (nx * dx)
    z, y, x = np.ogrid[:nz, :ny, :nx]
    r2 = (x*dx - mid_x*dx)**2 + (y*dx - mid_y*dx)**2 + (z*dx - mid_z*dx)**2
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="SATP+Higgs 3D Comparison Simulation")

...
required=['work_package.json','syntax_semantics_closure_draft.md','validation_matrix.json','frozen_typed_abstract_syntax.json','typed_syntax_fixtures.json','typed_syntax_validation_report.json','finite_model.json','truth_condition_fixtures.json','finite_model_truth_report.json','substitution_composition_report.json','inference_soundness_report.json','operator_algebra_report.json','binder_extension_schema.json','binder_substitution_report.json','model_class_comparison_report.json']
def load(name):
    return json.loads((ROOT/name).read_text(encoding='utf-8'))
def main():
    presence={n:(ROOT/n).exists() for n in required}
    syntax=load('typed_syntax_validation_report.json'); truth=load('finite_model_truth_report.json'); sub=load('substitution_composition_report.json'); inf=load('inference_soundness_report.json'); alg=load('operator_algebra_report.json'); binder=load('binder_substitution_report.json'); models=load('model_class_comparison_report.json'); wp=load('work_package.json')
    reruns=[]
    for script in ['validate_typed_syntax_fixtures.py','evaluate_finite_model.py','test_substitution_composition.py','test_inference_soundness.py','test_operator_algebra.py','test_binder_substitution.py','model_class_comparison.py']:
    ...
    gaps={
      ...
      ,'model_sensitivity': {'state':'observed','evidence':'model_class_comparison_report.json','remaining':'no model-independent semantics established'}
    }
    ...
    status='PASS_BOUNDED_REVIEW' if all(presence.values()) and all(boundaries.values()) and syntax['status']=='PASS' and truth['status']=='PASS' and sub['status']=='PASS' and inf['status']=='PASS' and alg['status']=='PASS' and binder['status']=='PASS' and models['status']=='PASS' else 'FAIL_REVIEW'

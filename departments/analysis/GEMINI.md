...
Required closeout files are `campaign_summary.md`, `campaign_summary.json`, `proof_readiness_dashboard.json`, `research_debt_progress.json`, `next_campaign.json`, `human_review_checklist.md`, `obligation_summary.md`, `actionable_tasks.json`, `executive_dashboard.json`, `unlock_analysis.json`, `cost_benefit_report.json`, `agent_handoff.json`, and `repository_health.json`. Every campaign begins with a one-page executive dashboard and every actionable task must state its unlocks, blocked objects, future campaigns enabled, effort, compute cost, repository value, risk, confidence, and recommended specialist handoff. Rank tasks by repository unlock value, proof impact, dependency reduction, debt reduction, and governance importance. These are analysis products, not promotion authority. A campaign may not report success merely because it generated discoveries; objective completion, evidence quality, termination quality, and unresolved debt must be assessed independently.
